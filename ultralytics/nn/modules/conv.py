...
class FAM(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv2 = CBA(c1, c_, 1, 1)

        self.conv_lighting = nn.Sequential(
            AConv(c_, c_, 1, 1),
            AConv(c_, c_, 3, 1, act=False)
        )
    ...
    def forward(self, x):
        ...
        return self.cv3(
            torch.cat((y, x1), dim=1)
        )
    
class Conv(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""

    default_act = AcReLU()  # default activation
...
class SPDConv(nn.Module):
    ...
    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension
        # self.cv = Conv(c*4, c, 1, 1)

    def forward(self, x):
         out= torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1)
         # out=self.cv(out)
         # print('space_to_depth',out.shape)
         return out
